...
if(isset($_POST['add_performance_testing']) && $have_write_permission) {
  ...
  if($retval != NULL) {
    ...
    //we add the entry.
    $insert_query = "INSERT INTO performance_testing (student_id,test_name,date,results,file,filename) VALUES (" . mysql_real_escape_string($student_id) . ",'" . addslashes($_POST['test_name']) . "',";
     $insert_query = $insert_query . "'" . mysql_real_escape_string($_POST['date']) . "','" . addslashes($_POST['results']) . "','$content','$fileName')";
